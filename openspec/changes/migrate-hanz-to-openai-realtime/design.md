...
The fixed configuration uses `gpt-realtime-2.1`, audio output, the source-owned Hanz instructions, `marin`, near-field input noise reduction, input transcription, low-eagerness semantic VAD with automatic response creation and interruption, automatic tool choice, bounded output tokens, and the exact source-owned nine function schemas. The top-level client secret expires 60 seconds after creation to constrain credential replay. The Worker also returns a source-owned 300-second session deadline derived from its server clock. The mint endpoint accepts no browser configuration, and the shipped browser runtime sends no `session.update` overrides.
...
- **An ephemeral client can technically send Realtime `session.update` events** -> Accept no configuration at the mint endpoint, send no updates from the shipped runtime, mint a 60-second client secret, enforce the five-minute Hanz limit in the shipped monotonic runtime, and keep all application mutation authority in the strict local dispatcher. A trusted sideband/proxy and server-authoritative deadline are intentionally reserved for Tutor's separate workflow.
