...
**Current state:** all product code for this major update is merged. This docs-only closure branch reconciles the plan, log, and summary; after its green merge, the exact final `main` will be deployed and verified live without bypassing Cloudflare protections.
