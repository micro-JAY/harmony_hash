...
**Current state:** feature code, QA, and independent review are complete on the stacked branch pending final OpenSpec verification, commit, and push. A fresh status-only production check still returns 502, so S.2 PR #29 remains untouched and draft; its signed-URL endpoint must return 200 before merge, with no key or signed URL ever printed.
