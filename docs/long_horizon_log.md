...
**Review remediation:** addressed all eight Codex threads (two P1, six P2), including the fresh exact-head re-review. Floating dimensions now use documented design tokens and floating surfaces use component-owned semantic inline styles. Live pins re-clamp from rendered bounds against visual-viewport width, height, and offsets after resize or scroll, falling back to the layout viewport; only the manual drag handle disables touch panning. Tour startup closes the controlled chord browser so the preset target exists. Guitar diagrams report explicit loading, ready, and error states so failed MIDI sources surface an actionable Share-panel error. The browser context stacks without horizontal overflow at both 320 and 360 pixels.

**Verification:** production build and lint pass; Vitest passes 63 files / 1,348 tests; the exact full Chromium matrix passes 203/203 in 2.1 minutes; strict OpenSpec validates all 33 items. Focused coverage proves the 1,500 ms boundary, preview cancellation, Guitar and Piano pin controls, local pin modification, layout and offset-visual-viewport recovery, touch scrolling, bounded dragging, zero audio-context construction, Run/Modify/workspace persistence, guided-tour target restoration, selected-voicing MIDI bytes, explicit diagram failure, no tempo event, download preparation/error states, responsive 320/360/375-pixel English/Japanese control alignment, preset hiding/restoration, and existing composer insertion behavior.

**Current state:** `add-chord-preview-pins-midi-export` is 7/7 complete on branch `feat/hasher-hover-pins-midi-share`. PR #98 is ready for review; its integrated head preserves the newer OpenAI Realtime, privacy, control-alignment, and timeline-modification work from `main`. No deployment or OpenSpec archive occurred.
