...
| S.2 | **Agent Builder Recovery** — migrate progression generation to OpenAI Responses, restore signed-only ElevenLabs auth, and compact/responsive builder actions. | `fix/agent-builder-companion-ui` | `restore-agent-builder-experience` | `progression-agent`, `voice-companion`, `progression-input`, `app-shell` | **Blocked on provider config** (PR [#29](https://github.com/micro-JAY/harmony_hash/pull/29) is draft, CI-green, and cleanly mergeable; rotated ElevenLabs key is present but lacks `convai_write`, so live signed-url verification returns 502. Merge/production/archive wait for a 200.) |
