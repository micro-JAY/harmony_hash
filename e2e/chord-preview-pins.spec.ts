...
test("delays preview, promotes a silent full card, and preserves the pin across app workspaces", async ({
  ...
  const pin = page.getByTestId("pinned-chord-card").filter({
    has: page.getByRole("heading", { name: "Cmaj7" }),
  });
  await expect(pin).toBeVisible();
  await expect(pin.getByRole("button", { name: "Modify" })).toBeVisible();
  await expect(pin.getByRole("group", { name: "Guitar labels for Cmaj7" })).toBeVisible();
  ...
  await pin.getByRole("button", { name: "Dismiss pinned chord card: Cmaj7" }).click();
