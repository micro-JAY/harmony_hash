...
| 2.2.x.a | **Jazz suggestion-overlay mode** (deferred from 2.2). Voice-leading-aware scoring + tritone-sub awareness + ii-V detection + variable-strength border-glow. | — | `suggestion-overlay-jazz` | extends `chord-grid-suggestions` | Needs its own scoring engine + design pass. |
...
| 2.3 | **Improv Insight** — progression-aware scale suggestions with motion/tension/palette/style metadata. Per-chord and whole-progression tabs. Builds on `src/lib/theory/`. | `feat/improv-insight` | `improv-insight` | new capability `improv-insight`; consumes `src/lib/theory/` | [#37](https://github.com/micro-JAY/harmony_hash/pull/37) **Ready for review.** CI-green, mergeable, and intentionally unmerged. |
| 2.4 | **Common Progressions library expansion.** Audit `src/data/progressions.ts` and `docs/hh-library.md` against the inspiration "Common Progressions" examples; fill named gaps. Drop selected progression into the timeline. | `feat/progression-library-expansion` | — (normal planning) | `progression-library`, `progression-browser` | Draft [#38](https://github.com/micro-JAY/harmony_hash/pull/38) **Ready for review.** Audit found both named examples already shipped; 62 presets resolve across all 12 keys, and source/docs hierarchy plus picker behavior now have regression coverage. No speculative data expansion. |
...
| 2.8 | **Guitar/bass fretboard — first-class standard-tuning map.** Builder/Fretboard workspace navigation; Guitar/Bass, root, seven modes, Intervals/Notes; horizontal frets 0–15, role colors, exact note semantics, arrow navigation, internal mobile scrolling. | `feat/guitar-fretboard` | `guitar-fretboard-view` | new `guitar-fretboard`; extends `app-shell` and shared `src/lib/theory/` | Draft [#31](https://github.com/micro-JAY/harmony_hash/pull/31) **In Progress.** Tested and pushed, stacked on 2.11; do not merge ahead of the blocked S.2 chain. |
| 2.8.x.a | **Fretboard tuning + handedness expansion.** Guitar Standard/Drop D/DADGAD/Open G, bass Standard/Drop D/BEAD, per-instrument tuning memory, and left-handed visual-axis reversal. | `feat/fretboard-tunings-handedness` | `fretboard-tunings-handedness` | extends `guitar-fretboard` | Draft [#32](https://github.com/micro-JAY/harmony_hash/pull/32) **In Progress.** Tested and pushed, stacked on 2.8; do not merge ahead of the blocked S.2 chain. |
| 2.8.x.b | **Fretboard patterns + chord overlay.** All/CAGED/3NPS position filtering and dictionary-valid chord-tone overlay on the current scale. | `feat/fretboard-patterns-chord-overlay` | `fretboard-patterns-chord-overlay` | extends `guitar-fretboard`; consumes chord data | Draft [#33](https://github.com/micro-JAY/harmony_hash/pull/33) **In Progress.** Tested and pushed, stacked on 2.8.x.a; archive after eligible merge, then use normal planning without new OpenSpec changes. |
...
| S.2 | **Agent Builder Recovery** — migrate progression generation to OpenAI Responses, restore signed-only ElevenLabs auth, and compact/responsive builder actions. | `fix/agent-builder-companion-ui` | `restore-agent-builder-experience` | `progression-agent`, `voice-companion`, `progression-input`, `app-shell` | PR [#29](https://github.com/micro-JAY/harmony_hash/pull/29) **Merged and deployed.** Cloudflare records tagged production deployments for the merged recovery and completed feature stack. A fresh signed-URL status check remains unobserved because the custom-domain challenge returns 403 before the Worker; do not treat that edge response as a provider failure or claim live voice success without a Worker-level 200. |
