...
| 2.8.x.b | **Fretboard patterns + chord overlay.** All/CAGED/3NPS position filtering and dictionary-valid chord-tone overlay on the current scale. | — | `fretboard-patterns-chord-overlay` | extends `guitar-fretboard`; consumes chord data | Explicit follow-up after the shared degree palette and tuning model settle. |
