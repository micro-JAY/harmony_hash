...
| 2.8 | **Guitar/bass fretboard — first-class standard-tuning map.** Builder/Fretboard workspace navigation; Guitar/Bass, root, seven modes, Intervals/Notes; horizontal frets 0–15, role colors, exact note semantics, arrow navigation, internal mobile scrolling. | `feat/guitar-fretboard` | `guitar-fretboard-view` | new `guitar-fretboard`; extends `app-shell` and shared `src/lib/theory/` | **In Progress.** Lazy-loaded implementation, responsive QA, tests, and independent review complete; stacked delivery pending. |
