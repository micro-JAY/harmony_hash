...
## 2026-07-21 10:06 JST — OpenAI and Hanz production recovery

The live SPA and Worker remained reachable, but `/api/health` reported `openaiApiKey: false` and both `/api/progression` and `/api/voice/signed-url` returned HTTP 500. Version metadata identified the shared root cause: the 2026-07-20 Workers Build promoted `dbfcc3bc-6126-45db-8c28-6997cd4b5372` at 100% traffic with rate-limit, assets, and voice-agent bindings but zero secrets. The last known-good Worker version carried both provider secrets, ruling out a browser or upstream-provider regression.

Restored only the existing `OPENAI_API_KEY` and `ELEVENLABS_API_KEY` values from the gitignored local secret file. Version `7944f8dc-b304-4af1-9466-83143aec88be` was read back with exactly both secret names before receiving 100% traffic. Post-recovery safe probes returned HTTP 200 for health, real OpenAI progression generation, and ElevenLabs signed-URL minting. In the production browser, the builder changed from `Service unavailable` to `API ready` and generated `Cmaj7`, `Am7`, `Dm7`, `G7` with a valid rationale and rendered cards. The silent synthetic-microphone Hanz smoke connected in voice mode, received 9 audio packets, executed `replace_progression` with `Fmaj7`, `Gm7`, `C7`, `Fmaj7`, and disconnected cleanly.

The focused source guard declares exactly those two provider names under Wrangler `secrets.required`; a direct configuration test and Worker operations note prevent a secretless upload/deploy from passing unnoticed. Production build and lint pass; Vitest passes 55 files / 1,312 tests; all 182 Chromium scenarios pass in 2.3 minutes; strict OpenSpec, Wrangler dry-run packaging, diff hygiene, page identity, nonblank content, framework-overlay absence, interaction state, and an empty warning/error console pass. The source diff is limited to Wrangler configuration, its direct test, Worker operations guidance, OpenSpec, and this ledger. No product UI, HASHER, TUNE TOOLBOX, FRET FINDER, provider prompt, agent configuration, music theory, or public route changed. Next: publish a focused draft PR and require exact-head CI; no merge, additional deployment, secret rotation, or OpenSpec archive is authorized.

---

## 2026-07-21 12:06 JST — HASHER contextual note-tooltip and selector gate

Removed the fixed twelve-degree HASHER legend and moved its theory vocabulary into one shared chromatic presentation helper. Every played Guitar circle, open-string marker, and per-string barre marker now exposes the displayed note, chord degree, and full role through a compact pointer/focus tooltip and matching accessible name. Active Piano keys use the same contract without changing interval colors, voicings, keyboard geometry, playback, or card display modes. The global selector now sits directly beneath and right-aligned with Run; its visible text is replaced by the existing Lucide guitar and keyboard symbols, with the requested dark-on-light selected treatment and muted-grey inactive treatment.

**Rendered proof:** the in-app browser composed `Bsus4`, verified six individually named Guitar targets including the per-string barre marker, switched through the icon selector, and verified three Piano targets. A browser pass caught and fixed the Piano wrapper clipping the popup. Desktop 1500×900 and mobile 375×812 screenshots were inspected against the supplied builder/selector and dense-guitar references; the selector remains aligned beneath Run, no fixed legend remains, the tooltip is contained and readable, and the mobile document has no horizontal overflow. The accepted copy, Tonari palette, typography, chord-card container model, and existing controls remain unchanged.
...
## 2026-08-03 09:40 JST — Hanz OpenAI Realtime migration live gate

Replaced Hanz's ElevenLabs browser SDK, signed-URL route, agent provisioner, public agent configuration, and provider-specific tests with a source-owned OpenAI Realtime WebRTC runtime. The Worker now mints only a 60-second client secret for the fixed `gpt-realtime-2.1` session and returns a Worker-clock reference for Hanz's independent five-minute browser deadline. The standard `OPENAI_API_KEY` remains Worker-only; neither standard nor ephemeral credentials, authorization headers, SDP, or provider payloads are logged or persisted. A live API correction moved the credential TTL to top-level `expires_after`; client-secret expiry constrains new connection creation and is not treated as an active-call deadline.

The browser retains the existing Hanz panel, labels, focus behavior, permanent mount, and close/reopen continuity. It owns microphone and peer rollback, remote audio playback, item-ID transcripts, interruption state, inbound-audio health, explicit cancellation, and a monotonic deadline. The strict local dispatcher supports only the nine source-owned progression tools, validates every argument, deduplicates call IDs, and returns tool output only for completed function calls.

**Verification:** a synthetic-microphone Chromium session made exactly one Worker secret request and one OpenAI Realtime call, received 175 remote audio packets, executed `replace_progression` with exactly `Fmaj7`, `Gm7`, `C7`, `Fmaj7`, preserved the same peer and data channel across panel close/reopen, then closed the peer/channel and ended the microphone track. The harness records only bounded pass/fail counters and transport states. Production build and lint pass; Vitest passes 57 files / 1,317 tests; the complete sequential Chromium matrix passes 188/188; Wrangler dry-run packages the expected rate-limit and asset bindings; all 27 OpenSpec items strict-validate; dependency integrity and diff hygiene pass; the dependency audit reports zero vulnerabilities.

**Rollback and remaining release boundary:** source rollback is a commit revert to the previous ElevenLabs integration, while the immutable prior deployment and provider secret should remain available only for the normal rollback window. No Worker deployment, secret mutation, provider deletion, merge, or OpenSpec archive occurred. Before publication, run the complete unit/browser/build/Worker/OpenSpec/audit gates, complete an independent repository security scan, publish only a draft PR, and wait for Codex review suggestions. Deployment-time secret verification and retirement of the legacy provider secret remain release-only actions requiring separate authorization.
...
Reordered HASHER to Describe, Build Your Own, the Browse/Key/Mode/instrument rail, then Presets. Key and Mode use the compact browser-scoped rail, and the preset section unmounts while the browser is open before restoring its prior selection state when closed. Updated desktop, mobile, Harmony, and Improv visual baselines were inspected against the requested hierarchy.

**Review remediation:** addressed all twenty Codex threads (four P1, sixteen P2) across the initial and exact-head reviews. Floating placement now resolves documented semantic width, conservative placement-height, viewport/pointer gap, and toolbar-height tokens from `rem` or `px` values at runtime, and remeasures after root-font or theme-attribute changes so placement, rendered-bound clamping, cascade distance, and minimum exposed height cannot drift from CSS geometry. Floating surfaces use component-owned semantic inline styles. Live pins re-clamp from rendered bounds against visual-viewport width, height, and offsets after resize, scroll, or drag completion, falling back to the layout viewport; their CSS width and height limits track that same live visual viewport and retain one complete token-sized toolbar strip through extreme shrink. Repeated pins cascade from each existing card's rendered post-drag/post-clamp position so every drag handle remains reachable, contracting toward the visible bottom edge when a compact viewport has no full-card travel. The Share dialog uses the semantic dropdown layer above pins, while previews render above existing pins for deliberate duplication. Only the manual drag handle disables touch panning. Pending hover intent is canceled when Hasher becomes inactive, and preview entrance motion is suppressed for reduced-motion users. Tour startup closes the controlled chord browser so the preset target exists. Guitar diagrams report explicit loading, ready, and error states so failed MIDI sources surface an actionable Share-panel error; reselecting the active Guitar option no longer discards those cached voicings. The browser context stacks without horizontal overflow at both 320 and 360 pixels.

**Verification:** production build and lint pass; Vitest passes 63 files / 1,349 tests; the exact full Chromium matrix passes 207/207 in 2.4 minutes; strict OpenSpec validates all 33 items. Focused coverage proves the 1,500 ms boundary, early-leave and inactive-workspace cancellation, reduced-motion rendering, Guitar and Piano pin controls, token-driven Piano width, pointer placement, cascade distance, edge clamping, and toolbar exposure at default and live 20-pixel root fonts, local pin modification, layout and offset-visual-viewport recovery after resize and drag, live visual-viewport card sizing, minimum toolbar recovery after an extreme viewport shrink, collision avoidance from a dragged live position, independently reachable repeated Piano pins at 360×480, Share-panel pointer precedence over overlapping pins, touch scrolling, bounded dragging, zero audio-context construction, Run/Modify/workspace persistence, guided-tour target restoration, selected-voicing MIDI bytes, same-Guitar reselection stability, explicit diagram failure, no tempo event, download preparation/error states, responsive 320/360/375-pixel English/Japanese control alignment, preset hiding/restoration, and existing composer insertion behavior.

**Current state:** `add-chord-preview-pins-midi-export` is 13/13 complete on branch `feat/hasher-hover-pins-midi-share`. PR #98 is ready for review; its integrated head preserves the newer OpenAI Realtime, privacy, control-alignment, and timeline-modification work from `main`. No deployment or OpenSpec archive occurred.
