...
- [x] 5.1 Create `components/ProgressionInput.tsx` with free-text input field and Run button (Enter key support)
- [x] 5.2 Add preset progression picker dropdown with grouped categories (Major, Aeolian, Harmonic Minor, Modal) and all specified progressions
- [x] 5.3 Add key selector dropdown with all 12 chromatic keys
- [x] 5.4 Wire input modes — free-text submits directly, preset picker + key selector triggers transposition then display
- [x] 5.5 Add inline error display for unrecognized chord symbols

## 6. UI Components — Chord Cards

- [x] 6.1 Create `components/ChordCard.tsx` — container card with chord name heading and instrument-conditional rendering
- [x] 6.2 Implement guitar mode in ChordCard — render SVG via `<img>` tag, show variant cycling arrows (prev/next) when Variation Count > 1, display variant indicator ("2 / 5")
- [x] 6.3 Create `components/PianoKeyboard.tsx` — procedural 2-octave HTML/CSS keyboard (C3 to B4) with white and black keys
- [x] 6.4 Implement note highlighting in PianoKeyboard — highlight active notes with glow accent color, distinguish LH/RH for Drop 2 voicings
- [x] 6.5 Implement "Randomize All" button — visible only in guitar mode, randomizes variant selection for all cards simultaneously

## 7. UI Components — App Shell

- [x] 7.1 Create `components/Header.tsx` with "Harmony Hash" branding and Tonari Labs identity
- [x] 7.2 Create `components/InstrumentToggle.tsx` — Guitar/Piano toggle with design system styling
- [x] 7.3 Build `App.tsx` — compose all components in the page layout (Header → Input → Chord Cards)
- [x] 7.4 Implement responsive layout — flex-wrap grid with responsive card widths
