...
| POST | `/api/voice/client-secret` | Empty body. Returns a 60-second `{ clientSecret, expiresAt }` plus server-clock `{ serverNow, sessionEndsAt }` metadata for Hanz's 300-second browser deadline. |
