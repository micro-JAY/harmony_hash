...
- **[Risk] JavaScript placement geometry drifts from CSS after a token or root-font change.** → Resolve the semantic widths, conservative placement heights, viewport/pointer gaps, and toolbar height to pixels at runtime, observe root sizing/style changes, and pass that complete geometry into placement, cascade, available-height, and rendered-bound clamping.
- **[Risk] A compact viewport leaves no full-card cascade range or a pin blocks application popovers.** → Allow cascaded cards to contract toward the visual-viewport bottom while preserving a toolbar strip, and place the Share surface on the semantic dropdown layer above all pins.
- **[Risk] A later viewport correction or drag makes the stored pin geometry stale.** → Reserve the toolbar-height cascade step even when the old top falls below the new viewport, report each pin's rendered position after drag/clamp, and use those live positions for the next collision search.
- **[Risk] Reselecting the current instrument invalidates diagram-derived voicings without remounting their reporters.** → Treat same-instrument selection as a no-op; clear Guitar voicings only for a real instrument transition.
