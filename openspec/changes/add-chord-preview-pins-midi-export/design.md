...
- **[Risk] A pin falls outside the viewport after resize.** → Re-clamp each live pin from its rendered bounds on window, visual-viewport, and card resize while retaining the dedicated drag constraints; responsive tests cover viewport shrinkage.
