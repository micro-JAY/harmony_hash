...
| 2.3 | **Improv Insight** — progression-aware scale suggestions with motion/tension/palette/style metadata. Per-chord and whole-progression tabs. Builds on `src/lib/theory/`. | `feat/improv-insight` | normal planning (OpenSpec discontinued) | new capability `improv-insight`; consumes `src/lib/theory/` | PR [#37](https://github.com/micro-JAY/harmony_hash/pull/37) **In Review.** Implementation, independent review, desktop/mobile Browser QA, build, lint, 247 Vitest assertions, and 54 Playwright tests are green. |
